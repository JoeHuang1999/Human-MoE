...
from src.dataset.my_deepfashion_dataset_hand import DeepFashionDataset
...
def sample(model, scheduler, train_config, diffusion_model_config,
	...
	for count in range(len(dataset)):
		...
		pose_idx = count
		text_idx = count
		# text_prompt = ['1girl, A beautiful woman in a dark black shirt, realistic, high-quality']
		...
		for i in tqdm(range(diffusion_config['num_timesteps']-1, -ddim_step, -ddim_step)):
			...
			if i == 0:
				img.save(f"src/deepfashion/cond_text_image_samples/hand-source.png")
			else:
				pass
			img.close()
...
def run_hand(model, vae):
	parser = argparse.ArgumentParser()
	parser.add_argument('--config', dest='config_path',
	                    default='src/config/my_deepfashion_text_image_cond_hand.yaml', type=str)
