...
from src.dataset.my_deepfashion_dataset_face import DeepFashionDataset
import time
		...
		for i in tqdm(range(diffusion_config['num_timesteps']-1, -ddim_step, -ddim_step)):
			...
			if cf_guidance_scale > 1:
				...
				noise_pred = noise_pred_uncond + cf_guidance_scale * (noise_pred_cond - noise_pred_uncond) + \
					cf_guidance_scale * (noise_pred_text_cond - noise_pred_uncond) * 0
			...
			if not os.path.exists(f"src/deepfashion/cond_text_image_samples"):
				os.mkdir(f"src/deepfashion/cond_text_image_samples")

			if i == 0:
				img.save(f"src/deepfashion/cond_text_image_samples/face-source.png")
			else:
				pass
			img.close()
		end_time = time.time()
		print(f"Time taken for {diffusion_config['num_timesteps']} steps: {end_time - start_time} seconds")
...
def run_face(model, vae):
	parser = argparse.ArgumentParser()
	parser.add_argument('--config', dest='config_path',
	                    default='src/config/my_deepfashion_text_image_cond_face.yaml', type=str)
