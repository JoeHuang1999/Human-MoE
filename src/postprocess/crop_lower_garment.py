...
def run_crop_lower_garment():
    ...
    if contours:
        ...
        if h <= 256:
            ...
            image.save(f"{image_folder_name}/lower-garment-ori.png")
            parsing = Image.fromarray(parsing[y1:y2, x1:x2]).resize((256, 256))
            parsing.save(f"{image_folder_name}/lower-garment-parsing.png")
            pose = Image.fromarray(pose[y1:y2, x1:x2]).resize((256, 256))
            pose.save(f"{image_folder_name}/lower-garment-pose.png")
            ...
            image_background.save(f"{image_folder_name}/lower-ori.png")
            parsing = Image.fromarray(parsing[y1:y2, x1:x2]).resize((new_w, new_h))
            parsing_background.paste(parsing, ((256 - w) // 2, 0))
            parsing_background.save(f"{image_folder_name}/lower-parsing.png")
            pose = Image.fromarray(pose[y1:y2, x1:x2]).resize((new_w, new_h))
            pose_background.paste(pose, ((256 - w) // 2, 0))
            pose_background.save(f"{image_folder_name}/lower-pose.png")
        with open(f"{image_folder_name}/lower-garment-box.txt", "w") as f:
