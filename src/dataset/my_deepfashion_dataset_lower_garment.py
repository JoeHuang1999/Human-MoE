...
class DeepFashionDataset(Dataset):
	def __init__(self, split, im_path, im_size=256, im_channels=3, im_ext='jpg',
		...
		self.images, self.texts, self.masks, self.poses = self.load_images(
			im_path)
	...
	def load_images(self, im_path):
		...
		if 'text' in self.condition_types:
			captions_im = []
			with open(f"final/deepfashion/cond_text_image_samples/lower-garment-text.txt") as f:
				for line in f.readlines():
					captions_im.append(line.strip())
			texts.append(captions_im)
		if 'image' in self.condition_types:
			masks.append(f"final/deepfashion/cond_text_image_samples/lower-garment-parsing.png")
		if 'pose' in self.condition_types:
			poses.append(f"final/deepfashion/cond_text_image_samples/lower-garment-pose.png")
		if 'text' in self.condition_types:
