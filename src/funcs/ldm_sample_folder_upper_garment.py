...
from src.dataset.my_deepfashion_dataset_upper_garment import DeepFashionDataset
...
def sample(model, scheduler, train_config, diffusion_model_config,
	...
	for count in range(len(dataset)):
		...
		text_prompt = ['A heather gray, close-fitting, cropped halter top.']
		empty_prompt = ['']
		...
		for i in tqdm(range(diffusion_config['num_timesteps']-1, -ddim_step, -ddim_step)):
			...
			if i == 0:
				img.save(f"src/deepfashion/cond_text_image_samples/upper-garment-source.png")
			else:
				pass
			img.close()
...
def infer(args, model, vae):
	...
	getModelSize(vae)
	return model, vae
def run_upper_garment(model, vae):
	parser = argparse.ArgumentParser()
	parser.add_argument('--config', dest='config_path',
	                    default='src/config/my_deepfashion_text_image_cond_upper_garment.yaml', type=str)
