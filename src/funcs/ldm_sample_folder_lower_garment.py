...
from src.dataset.my_deepfashion_dataset_lower_garment import DeepFashionDataset
...
def sample(model, scheduler, train_config, diffusion_model_config,
	...
	for count in range(len(dataset)):
		...
		text_prompt = ['white background, simple background, solo, monochrome, black, no humans']
		empty_prompt = ['']
		...
		for i in tqdm(range(diffusion_config['num_timesteps']-1, -ddim_step, -ddim_step)):
			...
			if cf_guidance_scale > 1:
				...
				noise_pred = noise_pred_uncond + cf_guidance_scale * (noise_pred_cond - noise_pred_uncond) + \
					cf_guidance_scale * (noise_pred_text_cond - noise_pred_uncond)
			...
			if i == 0:
				img.save(f"src/deepfashion/cond_text_image_samples/lower-garment-source.png")
			else:
				pass
			img.close()
...
def infer(args, model, vae):
	...
	getModelSize(vae)
	return model, vae
def run_lower_garment(model, vae):
	parser = argparse.ArgumentParser()
	parser.add_argument('--config', dest='config_path',
	                    default='src/config/my_deepfashion_text_image_cond_lower_garment.yaml', type=str)
