...
class DeepFashionDataset(Dataset):
	...
	def load_images(self, im_path):
		...
		"""
		ims = []
		ims.append(f"final/deepfashion/cond_text_image_samples/face-ori.png")
		texts = []
		masks = []
		poses = []
		if 'image' in self.condition_types:
		...
		if 'text' in self.condition_types:
			captions_im = []
			with open(f"final/deepfashion/cond_text_image_samples/upper-garment-text.txt") as f:
				for line in f.readlines():
					captions_im.append(line.strip())
			texts.append(captions_im)

		if 'image' in self.condition_types:
			masks.append(f"final/deepfashion/cond_text_image_samples/upper-garment-parsing.png")

		if 'pose' in self.condition_types:
			poses.append(f"final/deepfashion/cond_text_image_samples/upper-garment-pose.png")
		...
		if 'pose' in self.condition_types:
			assert len(poses) == len(ims), "Condition Type Image but could not find sketches for all images"
	...
	def get_text(self, index, text_index=0):
		try:
			...
			print(f"file name: {self.get_name(index)}")
			return ""
